...
/*
 radius 0.5
 height 2
 angularResolution 60
 verticalResolution 1
 
 一周的弧度数为2πr/r=2π，360°角=2π弧度 1弧度约为57.3度
...
GeometryData generateCylinderWallGeometryData(float radius, float height, int angularResolution,
    ...
    for (int v = 0; v <= vertical; v++) {
        ...
        for (int a = 0; a <= angular; a++) {
            ...
            const float angle = af * angularInc; // 弧度
...
//                           .uv = simd_make_float2(af / angularf, vf / verticalf) };
            vtx[vertexIndex++] =
                (Vertex) { .position = simd_make_float4(x, y, z, 1.0),
                           .normal = simd_normalize(simd_make_float3(0.0, 0.0, 0.0)),
...
//GeometryData generatePieWallGeometryData(float radius, float height, int angularResolution,
//                                              int verticalResolution, float sectorAnglePercentage, float startAngle) {
//    // 垂直分辨率
//    const int vertical = verticalResolution > 0 ? verticalResolution : 1;
//    // 角度分辨率
//    const int angular = angularResolution > 2 ? angularResolution : 3;
...
//
//    // 每一分辩率的弧度 弧度=2pi/分辨率
//    const float angularInc = (2.0 * M_PI * sectorAnglePercentage) / (angularf - 1);
//    const float startAngular = 2.0 * M_PI * startAngle;
//    // 每一份高度的分辨率
//    const float verticalInc = height / verticalf;
//
//    // y便宜量
//    const float yOffset = -0.5 * height;
//    // 每次循环
//    const int perLoop = angular + 1 + 1;
//    // 顶点
//    const int vertices = (perLoop) * (vertical + 1);
//    // 三角形态
//    const int triangles = (angular + 1) * 2 * vertical;
//
//    // 顶点
//    Vertex *vtx = (Vertex *)malloc(vertices * sizeof(Vertex));
//    // 三角形
...
//        const float y = yOffset + vf * verticalInc;
//
//        for (int a = 0; a <= angular + 1; a++) {
//
//
//
//            if (a == 0 || a == angular + 1) {
//                vtx[vertexIndex++] =
//                    (Vertex) { .position = simd_make_float4(0, y, 0, 1.0),
//                               .normal = simd_normalize(simd_make_float3(0.0, 0.0, 0.0)),
//                               .uv = simd_make_float2(0, 0) };
//            } else {
//                const float af = (float)(a - 1);
//                const float angle = af * angularInc + startAngular; // 弧度
...
//                               .uv = simd_make_float2(af / angularf, vf / verticalf) };
//
//                if ((a == 1 || a == angular) && v == 0) {
//                    printf("wangzy %f \n", x);
//                }
//            }
//
//
//            if (v != vertical && a != angular + 1) {
//                const uint32_t index = a + v * perLoop;
//
//                const uint32_t tl = index;
//                const uint32_t tr = tl + 1;
...
////        printf("%f %f %f \n", vtx[i].position.x, vtx[i].position.y, vtx[i].position.z);
////    }
////
...
GeometryData generatePieWallGeometryData(float radius, float height, int angularResolution,
    ...
    const float yOffset = -0.5 * height;
    // 每次循环
    const int perLoop = angular * 2 + 1;
    // 顶点
    const int vertices = (perLoop) * (vertical + 1);
    // 三角形态
    const int triangles = (angular * 2) * 2 * vertical;
    ...
    for (int v = 0; v <= vertical; v++) {
        ...
        for (int a = 0; a <= angular * 2; a++) {
            ...
            if (a < angular) {
                ...
                const float af = (float)(a - b);
                const float angle = af * angularInc + startAngular; // 弧度
                const float x = cos(angle);
                const float z = sin(angle);
                vtx[vertexIndex++] =
                    (Vertex) { .position = simd_make_float4((radius - 0.4) * x, y, (radius - 0.4) * z, 1.0),
                               .normal = simd_normalize(simd_make_float3(x, 0.0, z)),
                               .uv = simd_make_float2(af / angularf, vf / verticalf) };
                b += 2;
            } else {
                const float af = (float)(0);
                const float angle = af * angularInc + startAngular; // 弧度
                const float x = cos(angle);
                const float z = sin(angle);
                vtx[vertexIndex++] =
...
//    for (int i = 0; i < vertices; i++) {
//        printf("wangzy %f %f %f \n", vtx[i].position.x, vtx[i].position.x, vtx[i].position.z);
//    }
//
//    for (int i = 0; i < triangles; i++) {
//        printf("wangzy %f %f %f \n", vtx[i].position.x, vtx[i].position.x, vtx[i].position.z);
...
//    bool flip = true;
//    float direction = 1.0;
//    for (int i = 0; i < 2; i++) { // 两层
//
//        for (int r = 0; r <= radial; r++) { // 计算有几层，目前0代表中心，1代表边缘。radial有多层的话另算
//            const float rf = (float)r;
//            const float rad = rf * radialInc;
//            for (int a = 0; a <= angular; a++) { //
//                const float af = (float)a;
...
//                    .normal = simd_make_float3(0.0, direction, 0.0), .uv = simd_make_float2(0, 0)};
...
//    printf("=====\n");
//
//    for (int i = 6; i < triangles; i++) {
//        TriangleIndices tmp = ind[i];
...
/*
 positon 位置
 normal 法线 在3dmax中“法线”用来标记“面”的中正反，有“法线”的一面是正面，默认可被渲染的那个面。
 UV 属性就是纹理UV坐标
 */
//GeometryData generatePieGeometryData(float radius, float height, int angularResolution,
//                                          int radialResolution, int verticalResolution, float piePercentage, float startAngle) {
//
//    const int radial = radialResolution > 0 ? radialResolution : 1;
//    const int angular = angularResolution > 2 ? angularResolution : 3;
//
...
//
//    const float radialInc = radius / radialf;
//    const float angularInc = (2.0 * M_PI * piePercentage) / (angularf - 1);
//    const float startAngular = 2.0 * M_PI * startAngle;
//
//    const int perLoop = angular + 1;
//
//    GeometryData geometry =
//        generatePieWallGeometryData(radius, height, angularResolution, verticalResolution, piePercentage, startAngle);
//
//    const int verticesPerCircle = perLoop * (radial + 1);
//    const int trianglesPerCircle = angular * 2 * radial;
//    const int vertices = 2 * verticesPerCircle + geometry.vertexCount;
...
//            const float rf = (float)r;
//            const float rad = rf * radialInc; // 半径
//            for (int a = 0; a <= angular * 2; a++) {
//                const float af = (float)a;
//                const float angle = af * angularInc + startAngular;
//                const float x = rad * cos(angle);
//                const float y = rad * sin(angle);
//
//                if (a == angular) {
//                    vtx[vertexIndex++] =
//                        (Vertex) { .position = simd_make_float4(0, direction * height * 0.5, 0, 1.0),
...
GeometryData generatePieGeometryData(float radius, float height, int angularResolution,
    ...
    const int verticesPerCircle = perLoop * (radial);
    const int trianglesPerCircle = angular * 2 * radial;
    const int vertices = 2 * verticesPerCircle + geometry.vertexCount;
    const int triangles = 2 * trianglesPerCircle + geometry.indexCount;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int r = 0; r <= radial; r++) {
            ...
            for (int a = 0; a <= angular * 2; a++) {
                ...
                if (r == radial && a != angular * 2) {
                    const uint32_t index = vertexOffset + a + (r - 1) * perLoop;
...
GeometryData generateTorusGeometryData(float minorRadius, float majorRadius, int minorResolution,
    ...
    const int angular = majorResolution > 2 ? majorResolution : 3; // 每条棱有几个面
...
GeometryData generateRingGeometryData(float minorRadius, float majorRadius, int minorResolution,
                                       int majorResolution, float percent, float startAngle) {
    const int slices = minorResolution > 2 ? minorResolution : 3; // 几条棱
    const int angular = majorResolution > 2 ? majorResolution : 3; // 每条棱有几个面
    ...
    const int triangles = angular * 2 * slices + (addWall ? 4: 0);
    ...
    for (int s = 0; s <= loopSlices; s++) {
        ...
        for (int a = 0; a <= angular; a++) {
            ...
            if (addWall && s == 0 && a == angular) {
                const uint32_t index = a + s * perLoop;
                const uint32_t tl = index;
                ind[triangleIndex++] = (TriangleIndices) { .i0 = tl, .i1 = tl - 1, .i2 = tl - 2 };
                ind[triangleIndex++] = (TriangleIndices) { .i0 = tl - 2, .i1 = tl - 3, .i2 = tl - 4 };
